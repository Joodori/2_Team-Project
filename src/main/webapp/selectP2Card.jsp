...
<%
int gameID = (int) session.getAttribute("gameID"); // 추가
...
.card-glass {
	...
	border-radius: 1rem;
	transition: transform .15s ease, box-shadow .2s ease, outline-color .2s
		ease;
	cursor: pointer;
	height: 100%;
}
...
<body>
	<div class="container-main mx-auto py-5">
		...
		<div class="hero mb-5">
			<div
				...
				<div>
					<span class="badge badge-mode px-3 py-2 mb-3">PLAYER 2 ·
						PICK 3</span>
					<h1 class="text-white fw-bold mb-2">카드 선택</h1>
					<div class="text-gray-400">
						원하는 카드를 최대 <strong>3장</strong>까지 선택하세요. 선택 후 하단의 <b>싸우러가기</b> 버튼으로
						전투로 이동합니다.
					</div>
				</div>
		...
		<!-- 카드 그리드 -->
		<div
			class="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-xl-5 g-4">
			<%
			...
			<div class="col">
				<div class="card card-glass h-100"
					...
					data-att="<%=card.getAtt()%>" data-hp="<%=card.getHp()%>"
					data-detail="<%=card.getDetail()%>" data-img="/pj2/<%=path%>">
					<img src="/pj2/<%=path%>" class="card-img-top"
						alt="<%=card.getCardName()%>">
					<div class="card-body">
						...
						<div
							...
							<span class="badge bg-danger fs-6 px-3 py-2"> ⚔ ATK :<strong><%=card.getAtt()%></strong>
							</span> <span class="badge bg-success fs-6 px-3 py-2"> 🖤 HP :<strong><%=card.getHp()%></strong>
							</span>
						</div>
						<!-- -->
						<br /> <span class="text-gray-300">특징</span>
						<div class="text-gray-400 small mt-2">
							<strong class="text-white"><%=card.getDetail()%></strong>
						</div>
					</div>
		...
		<div class="sticky-progress mt-4">
			<div
				...
				<div class="d-flex align-items-center gap-3">
					<span class="text-gray-300">선택: <strong id="pickCounter2">0</strong>
						/ <strong>3</strong></span>
					<button class="btn-next" type="button" id="btnNext" disabled
						onclick="goToFightPage()">
						싸우러가기
						<svg width="18" height="18" viewBox="0 0 24 24"
							fill="currentColor" class="ms-1">
	...
	<!-- 선택 데이터 POST -->
	<form id="p2Form" method="post" action="saveP2Cards.jsp" class="d-none">
	...
	<div class="position-fixed top-0 end-0 p-3" style="z-index: 1080">
		<div id="toastPick"
			class="toast align-items-center text-bg-dark border-0" role="alert"
  ...
  function syncUI(){
    ...
    selected.forEach(s=>{
      ...
      wrap.appendChild(makeInput('gameID'), ${gameID});  // 추가
